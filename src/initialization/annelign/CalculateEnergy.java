...
public class CalculateEnergy
{
    ...
    public static double calEner(ArrayList<Double> states, Cities cities)
    {
        ...
        energy = 0;
        for (int i=0; i<n-1; i++ ) {
            energy = energy + Metric.calculate(cities.getStateX((int)Math.floor(states.get(i))), cities.getStateX((int)Math.floor(states.get(i+1))),
                    cities.getStateY((int)Math.floor(states.get(i))), cities.getStateY((int)Math.floor(states.get(i+1))));

        }

        energy = energy + Metric.calculate(cities.getStateX((int) Math.floor(states.get(n-1))), cities.getStateX((int) Math.floor(states.get(0))),
